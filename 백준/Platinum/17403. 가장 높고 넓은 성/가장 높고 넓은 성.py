...
count = 0
floor = 0
while len(points_set) >= 3:
    floor += 1
    convex_hull = get_convex_hull(None, points_set)
    
    for point in convex_hull:
        if point in points_set: points_set.remove(point)
        if len(convex_hull) > 2: matchs[point] = floor

for point in points:
    print(matchs.get(point, 0), end = " ")