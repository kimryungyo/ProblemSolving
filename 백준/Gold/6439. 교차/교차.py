...
if __name__ == "__main__":
    ...
    for _ in range(n):
        a, b, c, d, e, f, g, h = ( Fraction(int(x)) for x in input().split() ) 
        p1, p2 = (a, b), (c, d)
        main = Linear(p1, p2)

        cube = [(e, f), (e, h), (g, f), (g, h), (e, f)]
        
        x_range = sorted([e, g])
        y_range = sorted([f, h])
        if is_value_in_ranges(a, [x_range]):
            if is_value_in_ranges(b, [y_range]):
                if is_value_in_ranges(c, [x_range]):
                    if is_value_in_ranges(d, [y_range]):
                        print("T")
                        continue
